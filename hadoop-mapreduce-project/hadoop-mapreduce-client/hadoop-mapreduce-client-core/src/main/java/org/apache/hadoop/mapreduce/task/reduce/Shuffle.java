...
public class Shuffle<K, V> implements ShuffleConsumerPlugin<K, V>, ExceptionReporter {
  ...
  public RawKeyValueIterator run() throws IOException, InterruptedException {
    ...
    // Start the map-completion events fetcher thread map结果拉取器
    final EventFetcher<K,V> eventFetcher = 
      new EventFetcher<K,V>(reduceId, umbilical, scheduler, this,
          maxEventsToFetch);
    eventFetcher.start();
    
    // Start the map-output fetcher threads 开启map结果的拉取器
    boolean isLocal = localMapFiles != null;
    final int numFetchers = isLocal ? 1 :
      jobConf.getInt(MRJobConfig.SHUFFLE_PARALLEL_COPIES, 5);//拉取线程并行度
    Fetcher<K,V>[] fetchers = new Fetcher[numFetchers];
    if (isLocal) {
      //如果是本地文件，则开启一个线程直接拉取，在拉取的过程中有合并文件，并且排序的过程
