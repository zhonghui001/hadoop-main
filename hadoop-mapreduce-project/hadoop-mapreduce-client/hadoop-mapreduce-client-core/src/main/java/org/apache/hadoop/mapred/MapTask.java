...
public class MapTask extends Task {
 ...
 private <T> T getSplitDetails(Path file, long offset) 
  ...
  void runNewMapper(final JobConf job,
    ...
    // rebuild the input split 重新构建split
    ...
    if (job.getNumReduceTasks() == 0) {
      ...
      output = new NewOutputCollector(taskContext, job, umbilical, reporter);//输出阶段 partition ,sort之类全部在这块
    }
  ...
  public static class MapOutputBuffer<K extends Object, V extends Object>
    ...
    // k/v accounting
    private IntBuffer kvmeta; // metadata overlay on backing store  kvbuffer中索引部分内容，一共为4*4个字节
    ...
    boolean spillInProgress;    //是否需要开始溢写磁盘
    int bufferRemaining;
    ...
    public void init(MapOutputCollector.Context context
      ...
      final float spillper =
        job.getFloat(JobContext.MAP_SORT_SPILL_PERCENT, (float)0.8); //设置一个阈值
      final int sortmb = job.getInt(JobContext.IO_SORT_MB, 100); //缓冲区大小
      indexCacheMemoryLimit = job.getInt(JobContext.INDEX_CACHE_MEMORY_LIMIT,
                                         INDEX_CACHE_MEMORY_LIMIT_DEFAULT);//设置索引的大小
      ...
      spillLock.lock();
      try {
        spillThread.start();//启动溢写线程，写文件，在写文件过程中，进行排序，combiner的执行
