...
public class ReduceContextImpl<KEYIN,VALUEIN,KEYOUT,VALUEOUT>
  ...
  public ReduceContextImpl(Configuration conf, TaskAttemptID taskid,
    ...
    this.input = input;//注意这个
    this.inputKeyCounter = inputKeyCounter;
    this.inputValueCounter = inputValueCounter;
    this.comparator = comparator;
    this.serializationFactory = new SerializationFactory(conf);
    this.keyDeserializer = serializationFactory.getDeserializer(keyClass);
    this.keyDeserializer.open(buffer);  //将序列化绑定buffer
    this.valueDeserializer = serializationFactory.getDeserializer(valueClass);
    this.valueDeserializer.open(buffer);
    hasMore = input.next();//默认是true
  ...
  public boolean nextKey() throws IOException,InterruptedException {
    //由于nextkeyIsSame默认值为false,所以会进入下一次循环，而在方法体中会更新nextKeyIsSame,所以下次会跳过
  ...
  public boolean nextKeyValue() throws IOException, InterruptedException {
     ...
     * 从之前绑定的buffer读取数据，然后返回key对应类型的数据
     * 这块填充了一组kv
     */
