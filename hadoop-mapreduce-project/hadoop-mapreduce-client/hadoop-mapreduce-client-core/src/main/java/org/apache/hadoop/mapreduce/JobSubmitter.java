...
class JobSubmitter {
  ...
  throws ClassNotFoundException, InterruptedException, IOException {
    ...
    JobID jobId = submitClient.getNewJobID(); //向yarn集群申请applictionId
    job.setJobID(jobId);
    Path submitJobDir = new Path(jobStagingArea, jobId.toString()); //临时文件目录
    ...
    try {
      ...
      /**
       * 1.将配置信息 写入hdfs
       * 2.后续由yarnRunner生成对应的ApplicationSubmissionContext(包含了很多)
       *
       *
       * 这儿将生成xml文件
       *
